...
요세푸스 문제와 비슷하다. 큐를 이용하여 구현한다. 시간 제한이 넉넉하므로, 큐에서 하나씩 빼고 넣는 과정을 직접 구현하였다.  
</br>

- ⚠️ `pow()` 함수는 `double` 자료형을 반환한다.   
- ⚠️ `%` 연산자는 __정수끼리의 연산만__ 가능하다.   
  따라서 `pow()` 계산 후에 그 반환 값을 `%` 연산하려고 할 때 에러가 났다. `pow()` 의 계산 결과를 (`long long`) 정수형으로 형변환하여 해결하였다.    
  *사실 `pow()` 는 큰 수를 다룰 때 부동소수점 정밀도 오차가 있다고 한다. 정수의 거듭제곱은 직접 곱하는 것이(`t*t*t`) 안전하다고...  
</br>

> ⚠️ 게임에서 제외되는 사람(탈락자) 는 `t^3` 번째 사람이다.  
> 따라서 큐에서 뺀 후 다시 맨 뒤로 보내주는 것은 __`t^3-1` 번째 사람까지__ 여야 한다. !!  
