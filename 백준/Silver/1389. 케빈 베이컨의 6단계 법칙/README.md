...
사이클이 있을 수도 있다.   
사람별로 각각 케빈 베이컨 수를 구한다. (각 노드에 대하여 다익스트라)  
DFS 보단 BFS 느낌이 나는데 ...  

---

- 💡 `BFS()` 의 활용  
  중간에 다익스트라 알고리즘을 수정해서 쓸까 생각도 했지만, `BFS()` 로 문제를 해결할 수 있다.  
  (사실 다익스트라의 `priority_queue` 를 `queue` 로 수정하면.. 가중치 비교 부분을 제외하고는 `BFS()` 와 비슷하게 된다).  

  > ⭐ __BFS의 특징:__ 특정 노드에서 출발해, __"동일한 반경의 노드들"__ 은 __"같은 거리"__ 를 갖는다.  
</br>

- ⚠️ 최단 거리 계산 방법  
  처음엔 거리 계산 변수(`dd`) 를 두어, 다음 노드(`next`) 를 정하고 큐에서 노드를 빼낼(`pop()`) 때마다 `dd` 가 1씩 증가하도록 했다.  
  🛑 그러나 잘못된 로직이었다! (같은 노드에서 온 노드들이 각기 다른 거리를 가지게 되었다)  

  > 💡 '거쳐온 간선의 수' 를 올바르게 저장하기 위해, __{다음 노드(`next`) 의 거리} = {(현재 노드 거리) + 1}__ 임을 이용한다.
