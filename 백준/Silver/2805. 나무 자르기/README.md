...
'절단기의 높이(인덱스)'에 따른 - '가져갈 수 있는 나무의 길이(값)' 를 배열로 저장하자!  
   
🛑1. 절단기의 높이 범위는 `0 ~ (나무의 최고 높이)` 이다. 문제에서 나무의 최고 높이는 10억이므로, 10억 칸짜리 배열을 만들면 --> 메모리 초과  
🛑2. 각 높이마다 가져갈 수 있는 나무의 길이를 계산하는 것도 만만치 않음.  

---

>💡 __`절단기의 높이(h)` 를 인덱스로 생각__ 하여 __이분(이진) 탐색__.   
> 그때그때 h 에 따른 '가져갈 수 있는 나무의 길이' 를 계산한다.  

---

- ⚠️ 자료형 주의 !! (`long long`)  
  나무의 최대 높이가 `10억` 이므로, 가져갈 수 있는 나무 길이의 합(`curr_length`)은 `int` 자료형의 범위(`약 21억`) 를 넘을 수 있다.  
  따라서 자료형을 `long long` 으로 설정한다.  
</br>
...
- ⚠️ 이분탐색의 종결조건  
  ...
  while(min < max) {}
  ```
  처음에 이분탐색의 종결조건을 위와 같이 설정했다가 틀렸다.  
  `min`과 `max`가 같아지는 순간이 정답인 상황이 될 수 있으므로 `min <= max` 로 설정하는 것이 바람직하다.  
</br>

- ⚠️ 나무를 과하게 가져가는 상황 (그러나 얼마나?)  
  처음에 `curr_length > M` 인 상황에서 범위만을 수정했다.  
  그러나 절단기의 높이에 따라 필요한 나무의 길이를 딱 맞추어 가져갈 수 없는 경우도 있다.  
  따라서 __나무를 과하게 가져가는 것 같아 보여도 그것이 최선의 높이(정답) 일 가능성이 있다.__  
  그러므로 나무를 과하게 가져가는 상황의 경우, 범위 수정과 함께, 그때의 절단기 높이(`mid`) 를 저장해 둔다.  
  ``` cpp
