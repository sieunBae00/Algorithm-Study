...
#### getline() 함수의 특성
- `getline(istream& is, string str, char dlim);` 지정된 delimiter를 만나기 **전까지**의 문자열을 읽어 str에 저장한다.  
  따라서 '.'을 delimiter로 설정하면, 그 이후의 `'\n'(줄바꿈)`도 str에 함께 저장된다.  
> 💡 마지막 "."을 만났을 때도, str에는 "."전까지의 문자열이 저장된다. 즉, str에는 `'\n'`이 저장되어 있음에 유의한다.  
> (🛑 처음에 무한루프의 종결조건을 `if(str.size() == 0)`로 설정했다가 문제가 생겼다. 이후 종결조건을 `if(str == "\n")`으로 변경하여 해결하였다.)
