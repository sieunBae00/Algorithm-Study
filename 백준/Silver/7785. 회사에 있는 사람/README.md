...
__삽입/삭제가 빈번__ 하게 일어나므로, 배열이 아닌 __트리__ 로 구현된 `set` 을 이용한다.  

- enter -> set 에 넣는다.  
- leave -> set 에 있는지 확인하고 있으면 지운다.  

---

#### ⚠️ `.find()` 함수의 반환값  

`.find()` 는 원소를 찾아 그 원소의 위치 (`iterator`) 를 반환한다. 이를 정수 값으로 생각하지 말자.  
정수와는 비교가 불가하고, 같은 `iterator` 를 반환하는 `.end()` 등과는 비교 가능하다.  
