...
처음에는 다솜이를 포함한 모든 후보의 득표수와 기호를 pair 로 묶어 ''우선순위 큐''에 넣을 생각을 했다.   
   
🛑 그런데 다솜이의 득표수를 수정할 때, 어떻게 `pq` 에서 뽑아낼 것인가?  

> 💡  다솜이는 우선순위 큐에 넣지 않는다!  
> 다솜이만 따로 빼두고, 나머지를 우선순위 큐에 넣어 득표수가 가장 많은 사람과 다솜이의 표 수를 비교한다.   
