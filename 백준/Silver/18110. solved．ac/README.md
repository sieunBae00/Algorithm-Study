...
문제에서 요구하는 사항은 어렵지 않다.  
위/아래의 얼마를 절사하고 남은 원소들로 평균을 내므로, 정렬이 필요하다.  
</br>

- ⚠️ `double` 자료형의 사용  
  
나눗셈 과정이 포함되어 있으므로, 절사/마지막 평균 부분에서는 `double` 자료형을 사용해야 함에 유의한다.   
`round()` 함수를 사용하지만, 처음부터 `int` 자료형을 사용한다면 나눗셈의 결과가 항상 __"정수(소수점 버림)"__ 가 되므로, 의도하지 않은 결과가 나올 수 있기 때문이다.   
(*물론 처음부터 `double` 자료형으로 선언하는 것 이외에도, 나눗셈 시 강제 형변환하거나, 실수 자료형으로 나누는 것도 가능하다)   

``` text
예) 
int n;  n = 10;  // n을 int 로 선언했을 때, n이 10이라면

- n이 double일 때:
10.0 * 3 / 20  =  30.0 / 20  =  1.5
round(1.5) = 2 (정답✅)

- n이 int일 때:
10 * 3 / 20  =  30 / 20  =  1 (1.5에서 소수점 버려짐)
round(1) = 1 (오답❌)
```
