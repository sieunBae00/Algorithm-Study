...
'안테나로부터 모든 집까지의 거리의 총 합이 최소' 가 되도록 하는 안테나의 위치는 여러 집들의 __중앙값__ 이다.  
정렬된 배열에서 가운데 인덱스의 값을 출력하면 된다.  
(이때, 여러 개의 값이 가능할 경우 가장 작은 값을 출력하므로, 정수끼리의 연산 결과는 정수가 된다는 점을 이용하여 자동으로 작은 값을 출력하도록 하였다)
