...
1. 들어오는 간선의 가중치를 배열에 저장하고, 에지 노드들 두 개씩의 가중치가 같은지 확인하고 높은 쪽으로 맞춘다.  
(같은 부모를 공유하는 에지 노드에 연결된 가중치는 서로 같아야 하므로)  
   
2. 부모-자식 간 인덱스를 이용해서 에지 노드부터 타고 올라가며 점검한다. << 이때 누적된 거리를 전달.  
(타고 올라가는 횟수를 하나씩 늘려가면서.)  
<br>
--> 처음엔 위와 같은 방식을 생각했다.  
🛑 구현을 하는데, 너무 복잡했다. 끙끙 앓다가 포기하고 다른 방법을 찾았다.  
  
> 💡엣지 노드부터 올라가면서 가중치를 더한 값을 비교하지 말고,  
> 부모 노드가 `왼쪽 자식` / `오른쪽 자식` 에게 최대 가중치의 합을 물어보는 방식으로 하자. (재귀)  

--> `bottom-up` 보단 `top-down` 느낌?  

---

### 느낀점

- 대체로 `bottom-up` 보단 `top-down` 이 효과적일 때가 많은 것 같다. (재귀의 힘)  
⭐ `top-down` 생각 많이하기  

- 너무 수식으로(만) 접근하려고 하면 잘못된 방향일 떄가 많은 것 같다.  
⭐ 수식보단 __직관__ 적으로 생각하기 !!  
