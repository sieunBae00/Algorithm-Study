...
1715번 카드정렬하기 문제와 아이디어는 똑같다. 작은 크기끼리 `greedy` 하게 우선적으로 합치는 것.  
다만 이 문제에서는 합치는 과정에서의 '비용'을 누적해서 구한다는 것이 포인트. (이때, 마지막에 남은, 하나로 합쳐진 최종 파일의 크기는 더하지 않음에 주의한다)  

---

#### ⚠️ 자료형 주의  
비용이 계속 누적되므로, 총 비용인 `ans` 뿐만 아니라 힙에 들어가는 파일의 크기도 `int` 자료형의 범위을 넘어갈 수 있다. !! (`long long` 자료형 써야 함)  
