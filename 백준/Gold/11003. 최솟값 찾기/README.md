...
범위의 크기: `i - (i-L+1) + 1` = `L` 이다. 이 범위를 옆으로 한 칸씩 끌고 가면서 그때그때의 최솟값을 구해야 함.  
  
그때그때  sort() 는 안될 것 같고, 우선순위 큐?  
아니면 배열로? 처음에 정렬 한번 하고 인덱스 하나씩만 빼고넣고?   
