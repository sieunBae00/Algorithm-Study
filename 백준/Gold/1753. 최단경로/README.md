...
'주어진 시작점에서 다른 모든 정점으로의 최단 경로를 구하는' 문제이므로, __다익스트라__ 알고리즘을 활용한다.  
(*다익스트라 알고리즘은 방향/무방향 그래프에 모두 이용할 수 있지만, 음수인 가중치가 존재할 경우 이용할 수 없다)  
입력되는 간선 정보를 저장하고, 주어진 시작점에서 다익스트라(함수) 를 거쳐, 반환되는 거리 배열(`vector`)을 출력 조건에 따라 출력하도록 구현하였다.  
