...
자동으로 정렬되는 '우선순위 큐(힙)' 를 사용한다.  
남은 카드 묶음들의 카드 수 차이가 적을수록 유리하므로, 작은 묶음들부터 차례로 합치는 `greedy` 한 방법을 사용한다.  
    

- ⚠️ 일반 배열을 정렬해서 사용하면 안 되나?  
--> 안 된다.  
일반 배열은 자동으로 정렬이 안 되고 `sort()` 를 해주어야 한다.  
그런데 합쳐진 카드 묶음이 원래의 카드 묶음보다 커질 수 있으므로, 매번 `sort()` 가 필요하다. -> 시간초과  
