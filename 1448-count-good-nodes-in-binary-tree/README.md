...
리프->루트 내림차순이면 줄줄이 good 일 텐데, 줄줄이가 아닌 이상 순서 상관 없으므로 다시 검사.     
    
생각해 보면 BFS 말고 DFS 도 가능. (DFS 도 부모 노드가 먼저 방문된다.)   
...
-> 부모의 good 여부가 자식의 good 여부를 결정하는 로직에 영향을 미치지 않으므로간단해진다. !! (그저 good 이면 정답 값에 하나를 더할 뿐이다.)   
